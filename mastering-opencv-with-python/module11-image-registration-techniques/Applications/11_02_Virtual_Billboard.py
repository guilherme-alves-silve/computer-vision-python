...
def mouse_handler(event, x, y, flags, data):
    # the order of clicks matter: left-upper, right-upper, right-bottom, left-bottom
    if data['done']:
        return
...
def get_roi_points(img):
    ...
    data = {'img': img.copy(), 'points': [], 'done': False}
...
# img_src = cv2.imread('sunset_panorama.jpg')
...
homography, status = cv2.findHomography(src_pts, roi_dst)

# Warp source image onto the destination image.
warped_img = cv2.warpPerspective(img_src, homography, (img_dst.shape[1], img_dst.shape[0]))
